...
void MeshRenderUI::TargetChanged() {
	mSelectedMaterialIdx = -1;
}
...
void MeshRenderUI::MaterialSlotPanel() {
	Ptr<CMeshRender> meshRender = GetTarget()->MeshRender();
	...
	for (UINT i = 0; i < primCount; ++i) {
		Ptr<AMaterial> mat = meshRender->GetMaterial(i);
...
void MeshRenderUI::MaterialPanel() {
	...
	Ptr<CMeshRender> meshRender = GetTarget()->MeshRender();
	Ptr<AMaterial> mat = meshRender->GetMaterial(index);
		...
		if (ImGui::BeginDragDropTarget()) {
			...
			if (PayLoad) {
				...
				if (pAsset->GetType() == EAsset::E_Material)
					meshRender->SetMaterial(index, dynamic_cast<AMaterial*>(pAsset.Get()));
			}
	...
	if (mat != nullptr) {
		...
		ImGui::TableNextRow();
		{
			...
			ImGui::TableSetColumnIndex(1);
			auto albedo = GetTarget()->MeshRender()->GetAlbedo(index);
			ImGui::SetNextItemWidth(-FLT_MIN);
			if (ImGui::ColorPicker3("##Albedo", albedo.data())) {
				GetTarget()->MeshRender()->SetAlbedo(index, albedo);
			}
		}
		ImGui::TableNextRow();
		{
			...
			ImGui::TableSetColumnIndex(1);
			auto roughness = GetTarget()->MeshRender()->GetRoughness(index);
			ImGui::SetNextItemWidth(-FLT_MIN);
			if (ImGui::SliderFloat("##Roughness", &roughness, 0.f, 1.f)) {
				roughness = std::clamp(roughness, 0.f, 1.f);
				GetTarget()->MeshRender()->SetRoughness(index, roughness);
			}
		}
		ImGui::TableNextRow();
		{
			...
			ImGui::TableSetColumnIndex(1);
			auto metalic = GetTarget()->MeshRender()->GetMetalic(index);
			ImGui::SetNextItemWidth(-FLT_MIN);
			if (ImGui::SliderFloat("##Metalic", &metalic, 0.f, 1.f)) {
				metalic = std::clamp(metalic, 0.f, 1.f);
				GetTarget()->MeshRender()->SetMetalic(index, metalic);
			}
		}
		ImGui::TableNextRow();
		{
			...
			ImGui::TableSetColumnIndex(1);
			auto specular = GetTarget()->MeshRender()->GetSpecular(index);
			ImGui::SetNextItemWidth(-FLT_MIN);
			if (ImGui::SliderFloat("##Specular", &specular, 0.f, 1.f)) {
				specular = std::clamp(specular, 0.f, 1.f);
				GetTarget()->MeshRender()->SetSpecular(index, specular);
			}
		}
	}
}
...
void MeshRenderUI::TexturePanel() {
	...
	ImGui::TableNextRow();
	{
		...
		ImGui::TableSetColumnIndex(1);
		auto albedoMap = GetTarget()->MeshRender()->GetAlbedoMap(index);
		...
		if (ImGui::BeginDragDropTarget()) {
			...
			if (PayLoad) {
				...
				if (pAsset->GetType() == EAsset::E_Texture)
					meshRender->SetAlbedoMap(index, dynamic_cast<ATexture*>(pAsset.Get()));
			}
	...
	ImGui::TableNextRow();
	{
		...
		ImGui::TableSetColumnIndex(1);
		auto normalMap = GetTarget()->MeshRender()->GetNormalMap(index);
		...
		if (ImGui::BeginDragDropTarget()) {
			...
			if (PayLoad) {
				...
				if (pAsset->GetType() == EAsset::E_Texture)
					meshRender->SetNormalMap(index, dynamic_cast<ATexture*>(pAsset.Get()));
			}
