#ifndef __D3D12DISOCCLUSIONBLUR3X3_HLSL__
#define __D3D12DISOCCLUSIONBLUR3X3_HLSL__
...
#include "./../../assets/Shader/Kernels.hlsli"

SVGF_DisocclusionBlur_RootConstants(b0)

Texture2D<DepthStencilBuffer::DepthBufferFormat>	gi_Depth				: register(t0);
Texture2D<Svgf::DisocclusionBlurStrengthMapFormat>	gi_BlurStrength			: register(t1);
Texture2D<GBuffer::RMSMapFormat>					gi_RMS					: register(t2);

RWTexture2D<Svgf::ValueMapFormat>					gio_Value				: register(u0);

// Group shared memory cache for the row aggregated results.
static const uint NumValuesToLoadPerRowOrColumn = Svgf::ThreadGroup::Default::Width + (FilterKernel::Width - 1);
...
void FilterHorizontally(in uint2 Gid, in uint GI) {
	...
	for (uint i = 0; i < NumRowsToLoadPerThread; ++i) {
		...
		// Filter the values for the first GroupDim columns.
		{
			...
			{
				if (GTid4x16.x < GroupDim.x && kcValue != Svgf::InvalidValue && 
						kcDepth != DepthStencilBuffer::InvalidDepthValue) {
			...
			for (uint c = 0; c < FilterKernel::Radius; ++c) {
				...
				if (cValue != Svgf::InvalidValue && 
						kcDepth != DepthStencilBuffer::InvalidDepthValue && 
						cDepth != DepthStencilBuffer::InvalidDepthValue) {
			...
			if (GTid4x16.x < GroupDim.x) {
				const float GaussianFilteredValue = gaussianWeightedSum > 1e-6 ? 
					gaussianWeightedValueSum / gaussianWeightedSum : Svgf::InvalidValue;
...
void FilterVertically(in uint2 DTid, in uint2 GTid, in float blurStrength) {
	...
	if (blurStrength >= 0.01f && kcDepth != DepthStencilBuffer::InvalidDepthValue) {
		float weightedValueSum = 0.f;
		float gaussianWeightedValueSum = 0.f;
		float weightSum = 0.f;
		...
		for (uint r = 0; r < FilterKernel::Width; ++r) {
			...
			if (rDepth != DepthStencilBuffer::InvalidDepthValue && 
					rFilteredValue != Svgf::InvalidValue) {
		...
		float gaussianFilteredValue = gaussianWeightSum > 1e-6 ? 
			gaussianWeightedValueSum / gaussianWeightSum : Svgf::InvalidValue;
        filteredValue = weightSum > 1e-6 ? weightedValueSum / weightSum : gaussianFilteredValue;
        filteredValue = filteredValue != Svgf::InvalidValue ? 
...
void CS(in uint2 Gid : SV_GroupID, in uint2 GTid : SV_GroupThreadID, 
	...
	float blurStrength;
	{
		if (GI == 0) FilteredResultCache[0][0] = Svgf::InvalidValue;
		...
		if (all(FilteredResultCache[0][0] == Svgf::InvalidValue)) return;
	}
...
#endif // __D3D12DISOCCLUSIONBLUR3X3_HLSL__