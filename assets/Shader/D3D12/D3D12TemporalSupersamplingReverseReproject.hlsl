...
#ifndef __D3D12TEMPORALSUPERSAMPLINGREVERSEREPROJECT_HLSL__
#define __D3D12TEMPORALSUPERSAMPLINGREVERSEREPROJECT_HLSL__
...
#include "./../../assets/Shader/D3D12/D3D12CrossBilateralWeights.hlsli"

ConstantBuffer<CrossBilateralFilterCB> cbReproject : register(b0);
...
Texture2D<GBuffer::NormalDepthMapFormat>         gi_ReprojectedNormalDepth  : register(t1);
Texture2D<GBuffer::VelocityMapFormat>            gi_Velocity                : register(t2);
Texture2D<Svgf::DepthPartialDerivativeMapFormat> gi_DepthPartialDerivative  : register(t3);
Texture2D<GBuffer::NormalDepthMapFormat>         gi_CachedNormalDepth       : register(t4);
Texture2D<Svgf::ValueMapFormat>                  gi_CachedValue             : register(t5);
Texture2D<Svgf::ValueSquaredMeanMapFormat>       gi_CachedValueSquaredMean  : register(t6);
Texture2D<Svgf::TSPPMapFormat>                   gi_CachedTspp              : register(t7);
Texture2D<Svgf::RayHitDistanceMapFormat>         gi_CachedRayHitDistance    : register(t8);

RWTexture2D<Svgf::TSPPMapFormat>                          go_CachedTspp              : register(u0);
RWTexture2D<Svgf::TSPPSquaredMeanRayHitDistanceMapFormat> go_ReprojectedCachedValues : register(u1);
...
void CS(in uint2 DTid : SV_DispatchThreadID) {
    ...
    if (!GBuffer::IsValidNormalDepth(ReprojNormalDepth) || Velocity.x > 1e2f) {
    ...
    weights = select(vCacheValues != Svgf::InvalidValue, weights, 0.f);
    const float WeightSum = dot(1, weights);

    float cachedValue = Svgf::InvalidValue;
...
#endif // __D3D12TEMPORALSUPERSAMPLINGREVERSEREPROJECT_HLSL__