#ifndef __D3D12CALCLOCALMEANVARIANCE_HLSL__
#define __D3D12CALCLOCALMEANVARIANCE_HLSL__
...
#include "./../../assets/Shader/D3D12/D3D12CalcLocalMeanVariance.hlsli"
...
void FilterHorizontally(uint2 Gid, uint GI) {
    ...
    for (uint i = 0; i < NumRowsToLoadPerThread; ++i) {
		...
		// Filter the values for the first GroupDim columns.
		{
            ...
            if (GTid4x16.x < GroupDim.x && value != Svgf::InvalidValue) {
                valueSum = value;
...
void FilterVertically(uint2 DTid, uint2 GTid) {
    ...
    go_LocalMeanVariance[Pixel] = numValues > 0 ? float2(Mean, variance) : Svgf::InvalidValue;
}
...
#endif // __D3D12CALCLOCALMEANVARIANCE_HLSL__