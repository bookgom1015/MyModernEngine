...
#include "./../../assets/Shader/D3D12/D3D12RaySorting.hlsli"
...
Texture2D<GBuffer::NormalDepthMapFormat>         gi_NormalDepthMap                       : register(t2);
Texture2D<GBuffer::NormalDepthMapFormat>         gi_RayDirectionOriginDepthMap           : register(t3);
Texture2D<RaySorting::RayIndexOffsetMapFormat>   gi_TexAOSortedToSourceRayIndexOffsetMap : register(t4);

RWTexture2D<Rtao::AOCoefficientMapFormat>        go_AOCoefficientMap                     : register(u0);
RWTexture2D<Rtao::RayHitDistanceMapFormat>       go_RayHitDistanceMap                    : register(u1);
...
float CalculateAO(out float tHit, in uint2 launchIndex, in Ray aoRay, in float3 surfaceNormal) {
    ...
    if (TraceAORayAndReportIfHit(tHit, aoRay, TMax, surfaceNormal)) {
        ...
        occlusion = AmbientOcclusion::OcclusionFunction(
            distZ, cbAO.SurfaceEpsilon, cbAO.OcclusionFadeStart, cbAO.OcclusionFadeEnd);
    }
...
void RTAO_RayGen() {
    ...
    float aoCoefficient = Rtao::InvalidAOCoefficientValue;
    ...
    go_RayHitDistanceMap[LaunchIndex] = Rtao::HasAORayHitAnyGeometry(tHit) ? tHit : cbAO.OcclusionRadius;
}
...
bool Get2DRayIndices(out uint2 sortedRayIndex2D, out uint2 srcRayIndex2D, in uint index1D) {
    const uint2 RayGroupDim = uint2(RaySorting::RayGroup::Width, RaySorting::RayGroup::Height);
...
void RTAO_RayGen_RaySorted() {
    ...
    go_RayHitDistanceMap[OutPixel] = Rtao::HasAORayHitAnyGeometry(tHit) ? tHit : cbAO.OcclusionRadius;
}
