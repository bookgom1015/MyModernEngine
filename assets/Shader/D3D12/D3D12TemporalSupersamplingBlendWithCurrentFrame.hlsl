...
#ifndef __D3D12TEMPORALSUPERSAMPLINGBLENDWIDTHCURRENTFRAME_HLSL__
#define __D3D12TEMPORALSUPERSAMPLINGBLENDWIDTHCURRENTFRAME_HLSL__
...
ConstantBuffer<BlendWithCurrentFrameCB> cbBlend : register(b0);

Texture2D<Svgf::ValueMapFormat>							gi_CurrentFrameValue						: register(t0);
Texture2D<Svgf::LocalMeanVarianceMapFormat>				gi_CurrentFrameLocalMeanVariance			: register(t1);
Texture2D<Svgf::RayHitDistanceMapFormat>				gi_CurrentFrameRayHitDistance				: register(t2);
Texture2D<Svgf::TSPPSquaredMeanRayHitDistanceMapFormat>	gi_ReprojTsppValueSquaredMeanRayHitDist		: register(t3);

RWTexture2D<Svgf::ValueMapFormat>						go_Value				: register(u0);
RWTexture2D<Svgf::ValueSquaredMeanMapFormat>			go_ValueSquaredMean		: register(u2);
RWTexture2D<Svgf::TSPPMapFormat>						go_Tspp					: register(u1);
RWTexture2D<Svgf::RayHitDistanceMapFormat>				go_RayHitDistance		: register(u3);
...
void CS(uint2 DTid : SV_DispatchThreadID) {
	...
	float value = isCurrentFrameValueActive ? gi_CurrentFrameValue[DTid] : Svgf::InvalidValue;
	const bool IsValidValue = value != Svgf::InvalidValue;
	float valueSquaredMean = IsValidValue ? value * value : Svgf::InvalidValue;
	float rayHitDistance = Svgf::InvalidValue;
	float variance = Svgf::InvalidValue;
...
#endif // __D3D12TEMPORALSUPERSAMPLINGBLENDWIDTHCURRENTFRAME_HLSL__