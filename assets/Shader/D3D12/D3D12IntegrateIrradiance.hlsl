...
Texture2D<AOMAP_FORMAT>                          gi_AOMap       : register(t6);

Texture2D<EnvironmentManager::BrdfLutMapFormat>                  gi_BrdfLutMap                : register(t7);
TextureCube<EnvironmentManager::DiffuseIrradianceCubeMapFormat>  gi_DiffuseIrradianceCubeMap  : register(t8);
TextureCube<EnvironmentManager::SpecularIrradianceCubeMapFormat> gi_SpecularIrradianceCubeMap : register(t9);
...
#include "./../../assets/Shader/D3D12/D3D12ReflectionProbe.hlsli"
...
HDR_FORMAT PS(in VertexOut pin) : SV_Target {
    ...
    const float3 GlobalSpecular = gi_SpecularIrradianceCubeMap.SampleLevel(
        gsamLinearClamp,
        ToLightW,
        Roughness * (float)(5 - 1)).rgb;
    
    float3 diffuseIBL  = SampleBlendedProbeDiffuse(selected, NormalW, GlobalDiffuse);
    float3 specularIBL = SampleBlendedProbeSpecular(
        selected,
        PosW.xyz,
        ToLightW,
        Roughness,
    ...
    if (gAoEnabled) {
        ...
        if (AOValue != Ssao::InvalidAOValue) ao = AOValue;
    }
